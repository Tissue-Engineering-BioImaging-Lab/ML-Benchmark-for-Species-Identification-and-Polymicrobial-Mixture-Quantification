# Train + run a Testing notebook end-to-end on the GPU with no browser, saving all
# outputs (plots, metrics, and model_predictions.csv) back into the Testing folder.
#
# Usage:   .\train_gpu_headless.ps1 CNN_mixed_noisy_separate.ipynb
#          .\train_gpu_headless.ps1 transformer_mixed_noisy_separate.ipynb
#
...
# (built from CNN/Dockerfile.gpu); no separate image is needed for Testing.
...
# Make sure the notebook actually exists in this Testing folder before launching Docker.
...
if (-not $existing) {
    ...
    Write-Host "    cd ..\CNN ; docker build -t $image -f Dockerfile.gpu . ; cd ..\Testing"
    exit 1
}
